  ...
  window.addEventListener('load', () => {
    ...
    class Congratulation {
        constructor($elm) {
          ...
          this.OPTION = {
            BREAK_POINT: 768,
            ADD_HEIGHT: 100, // 紙吹雪が降る範囲はエリア高＋この高さ
            SPEED_LEVEL: 15, // 小さくするほど速くなる
            PC_QUANTITY: 100, // BREAK_POINT以上での紙吹雪の総枚数
            SP_QUANTITY: 100 // BREAK_POINT以下での紙吹雪の総枚数
          }
          this.isPC;
        }
        updateDevice() {
          ...
          const timer = setInterval(() => {
            ...
            for (let i = 0; i < Math.floor(QUANTITY / 3); i++) {
              ...
              const bodyStyle = [
                ...
                'animation: ' + rotateKeyframe + ' 100ms linear infinite;',
                'animation-delay: ' + delay + 'ms;'
              ];
              // html生成
              html +=
    ...
    /**
     * mutetion observer event
     */
    const sectionTitles = document.querySelectorAll(".section-title");

    // カウンター要素をすべて取得
    const items = document.querySelectorAll( '.section-title' );
    ...
    const options = {
        rootMargin: "20px",
        threshold: 1,  // カウンター要素がすべて画面に表示されたとき(= 1)
    }
